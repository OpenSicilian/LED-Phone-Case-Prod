...
#define BRIGHTNESS   250      // 亮度（0-255，避免过亮导致供电不足）
#define MAXMODE      5        //工作模式最大值
...
const unsigned long LONG_PRESS_TIME = 2000;  //长按时间2秒
...
void runningMode() {
  ...
  {
    case 0:
    allColor(255, 0, 0);// 效果1：全亮红色
    break;

    case 1:
    allColor(0, 255, 0);// 效果2：全亮绿色
    break;

    case 2:
    allColor(0, 0, 255);// 效果3：全亮蓝色
    break;

    case 3:
    allColor(255, 255, 255);// 效果4：全亮白色
    break;

    case 4:
    strip.clear();  // 清空所有 LED
    runningLight(255, 255, 255);// 效果5：白色流水灯（逐个点亮）
    break;

    case 5:
    strip.clear();  // 清空所有 LED
    rainbow(30,64);//效果6：彩虹渐变（持续3秒）
    break;
